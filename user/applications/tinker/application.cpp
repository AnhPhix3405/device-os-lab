...
int tinkerWiFiSignalStrength(String command);
int tinkerBatteryLevel(String command); // Add this prototype
...
void setup()
{
    ...
    Particle.function("wifiSignal", tinkerWiFiSignalStrength);
    Particle.function("batteryLevel", tinkerBatteryLevel);

#ifdef ENABLE_FQC_FUNCTIONALITY
    BurninTest::instance()->setup();
#endif

    // Improved error handling for cloud connection
    if (!Particle.connected()) {
        Log.error("Cloud connection failed during setup. Retrying...");
        Particle.connect();
    }
}
...
void loop()
{
    ...
    // Improved error handling for cloud connection
    if (!Particle.connected()) {
        Log.warn("Cloud connection lost. Attempting to reconnect...");
        Particle.connect();
        waitFor(Particle.connected, 10000); // Wait for up to 10 seconds
        if (!Particle.connected()) {
            Log.error("Failed to reconnect to the cloud.");
        }
    }
}
...
/*******************************************************************************
 * Function Name  : tinkerWiFiSignalStrength
 * Description    : Gets the WiFi signal strength
 * Input          : None
 * Output         : None.
 * Return         : Returns the WiFi signal strength in dBm
 * Usage Example  : particle call <device_name> wifiSignal
 *******************************************************************************/
int tinkerWiFiSignalStrength(String command) {
...
/*******************************************************************************
 * Function Name  : tinkerBatteryLevel
 * Description    : Gets the battery level as a percentage
 * Input          : None
 * Output         : None.
 * Return         : Returns the battery level (0-100%) or -1 if not on battery
 * Usage Example  : particle call <device_name> batteryLevel
 *******************************************************************************/
int tinkerBatteryLevel(String command) {
    return getBatteryLevel(); // Call the function from request_handler.cpp
}
